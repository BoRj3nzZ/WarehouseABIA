...
public class Principal extends Thread{
	...
	private void crearOrden() {
		...
		do{
			try{
				...
				//car.setMovement(routeLeave, routeTake, itemId, takeItemPos, leaveItemPos);
				//car.startOrder(routeLeave, routeTake, itemId, takeItemPos, leaveItemPos);
	...
	private void startOrder(Vehiculo car, Posicion actualPos, Posicion takeItemPos, Posicion leaveItemPos, int itemId) {
		...
		car.move(route);	
		System.out.println("Ida ok, coge item: "+((WorkStation) takeItemPos).getListProductos().get(itemId));
		car.setItemInside(((WorkStation) takeItemPos).getArticulo(itemId));
		System.out.println("Item Taken");
		route = adminCaminos.getShortestRoute(takeItemPos, leaveItemPos);
		for(Posicion p:route) System.out.print(p.getId()+"-");
		car.move(route);
		car.deliverItemInside((WorkStation) leaveItemPos);
	}
