...
public class Almacen {
	...
	public void leerNewTasksDeOrdenDesdeDB(int orderId){
		...
		Posicion destination = order.getPosicionFinal();
		int prodId;
		try {
			connection = DBManager.getConnection();
			...
			while (resultSet.next()) {
				...
				order.addTaskToList(new Task(orderId, getArticleById(prodId), "to do", destination));
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	 ...
	 */
	public static void cambiarAsignacionCocheTaskDB(int orderID, int taskID, int newCarID) {
		Connection connection = null;
		PreparedStatement preparedStmt = null;
		try {
			connection = DBManager.getConnection();
			String query = "update task set idVEHICLE = ? where idPRODUCT = ? and idORDER = ?"; //
			preparedStmt = connection.prepareStatement(query);
	 ...
	 */
	public static void cambiarEstadoTaskDB(int orderID, int taskID, String newTaskStatus) {
		Connection connection = null;
		PreparedStatement preparedStmt = null;
		int statusID;
		...
		try {
			connection = DBManager.getConnection();
			String query = "update task set idSTATUS = ? where idPRODUCT = ? and idORDER = ?"; //
			preparedStmt = connection.prepareStatement(query);
			preparedStmt.setInt(1, statusID);
			preparedStmt.setInt(2, taskID);
			preparedStmt.setInt(3, orderID);
	 ...
	 *            The id of the product of the task that needs the change
	 */
	public static void cambiarFechaCompletarTaskDB(int orderID, int taskID) {
		Connection connection = null;
		PreparedStatement preparedStmt = null;
		Date date = new Date(Calendar.getInstance().getTimeInMillis());
		try {
			connection = DBManager.getConnection();
			String query = "update task set FINISHDATE = ? where idPRODUCT = ? and idORDER = ?"; //
			preparedStmt = connection.prepareStatement(query);
	 ...
	 */
	public static void cambiarEstadoOrderDB(Order order, String status) {
		Connection connection = null;
		PreparedStatement preparedStmt = null;
		int statusId;
